...
public:
	...
	bool insert(const T &key, const P &position)    //向B数中插入新结点key  
	{
		...
		else
		{
			...
			insertNonFull(m_pRoot, key, position);
			return true;
		}
	}
	...
	{
		cout << "--------打印B树关键字--------" << endl;
		displayInConcavo(m_pRoot, KEY_MAX);
	}
	bool contain(const T &key)   //检查该key是否存在于B树中  
	{
		return search(m_pRoot, key);
	}
	P getposition() {		//返回查找到的key的位置指针
		return pos;
	}
	void clear()                      //清空B树  
	{
...
private:
	...
	void splitChild(Node *pParent, int nChildIndex, Node *pChild)
	{
		...
		int i;
		for (i = 0; i<KEY_MIN; ++i)//拷贝关键字的值和地址指针  
		{
			pRightNode->keyValue[i] = pChild->keyValue[i + CHILD_MIN];
			pRightNode->position[i] = pChild->position[i + CHILD_MIN];
		}
		...
		for (i = pParent->keyNum; i>nChildIndex; --i)//将父节点中的nChildIndex后的所有关键字的值和地址指针和子树指针向后移一位  
		{
			pParent->pChild[i + 1] = pParent->pChild[i];
			pParent->keyValue[i] = pParent->keyValue[i - 1];
			pParent->position[i] = pParent->position[i - 1];
		}
		...
		pParent->position[nChildIndex] = pChild->position[KEY_MIN];//把节点的中间值的地址指针提到父节点  
	}

	//在非满节点中插入关键字  
	void insertNonFull(Node *pNode, const T &key, const P &position)
	{
		...
		if (pNode->isLeaf)      //pNode是叶子节点  
		{
			while (i>0 && key<pNode->keyValue[i - 1])   //从后往前，查找关键字的插入位置  
			{
				...
				pNode->position[i] = pNode->position[i - 1];  
				--i;
			}
			pNode->keyValue[i] = key;  //插入关键字的值  
			pNode->position[i] = position;  //插入关键字的地址指针  
		...
		else//pNode是内节点  
		{
			...
			insertNonFull(pChild, key, position);  //插入关键字到目标子树节点  
		}
	}
	...
	void displayInConcavo(Node *pNode, int count)const
	{
		if (pNode != NULL)
		{
			...
			for (i = 0; i<pNode->keyNum; ++i)
			{
				...
				//cout << pNode->keyValue[i] << "\t" << pNode->position[i] << endl;
	...
	void mergeChild(Node *pParent, int index)
	{
		...
		pChild1->position[KEY_MIN] = pParent->position[index];//将父节点index的地址指针下移
		int i;
		for (i = 0; i<KEY_MIN; ++i)
		{
			pChild1->keyValue[i + KEY_MIN + 1] = pChild2->keyValue[i];
			pChild1->position[i + KEY_MIN + 1] = pChild2->position[i];
		}
		if (!pChild1->isLeaf)
		{
			for (i = 0; i<CHILD_MIN; ++i)
		...
		if (i<pNode->keyNum&&key == pNode->keyValue[i])//关键字key在节点pNode中  
		{
			...
			else//pNode是个内节点  
			{
				...
				if (pChildPrev->keyNum >= CHILD_MIN)//节点pChildPrev中至少包含CHILD_MIN个关键字  
				{
					T prevKey; 
					P prevPosition;
					tie(prevKey, prevPosition) = getPredecessor(pChildPrev); //获取key的前驱关键字
					recursive_remove(pChildPrev, prevKey);
					pNode->keyValue[i] = prevKey;     //替换成key的前驱关键字
					pNode->position[i] = prevPosition;
					return;
				}
				else if (pChildNext->keyNum >= CHILD_MIN)//节点pChildNext中至少包含CHILD_MIN个关键字  
				{
					T nextKey;
					P nextPosition;
					tie(nextKey, nextPosition) = getSuccessor(pChildNext); //获取key的后继关键字  
					recursive_remove(pChildNext, nextKey);
					pNode->keyValue[i] = nextKey;     //替换成key的后继关键字  
					pNode->position[i] = nextPosition;
					return;
				}
			...
			if (pChildNode->keyNum == KEY_MIN)//只有t-1个关键字  
			{
				...
				if (pLeft&&pLeft->keyNum >= CHILD_MIN)//左兄弟节点至少有CHILD_MIN个关键字  
				{
					...
					for (j = pChildNode->keyNum; j>0; --j)
					{
						...
						pChildNode->position[j] = pChildNode->position[j - 1];
					}
					pChildNode->keyValue[0] = pNode->keyValue[i - 1];
					pChildNode->position[0] = pNode->position[i - 1];
					...
					pNode->position[i] = pLeft->position[pLeft->keyNum - 1];//pLeft节点中的最大关键字的地址指针上升到pNode中  
					--pLeft->keyNum;
				}
				else if (pRight&&pRight->keyNum >= CHILD_MIN)//右兄弟节点至少有CHILD_MIN个关键字  
				{
					...
					pChildNode->position[pChildNode->keyNum] = pNode->position[i];
					++pChildNode->keyNum;
					pNode->keyValue[i] = pRight->keyValue[0];//pRight节点中的最小关键字上升到pNode中
					pNode->position[i] = pRight->position[0];
					--pRight->keyNum;
					for (j = 0; j<pRight->keyNum; ++j)
					{
						pRight->keyValue[j] = pRight->keyValue[j + 1];
						pRight->position[j] = pRight->position[j + 1];
					}
					if (!pRight->isLeaf)
					{
	...
	std::tuple<T, P> getPredecessor(Node *pNode)//找到前驱关键字  
	{
		while (!pNode->isLeaf)
		{
			pNode = pNode->pChild[pNode->keyNum];
		}
		return make_tuple(pNode->keyValue[pNode->keyNum - 1], pNode->position[pNode->keyNum - 1]);
	}

	std::tuple<T, P> getSuccessor(Node *pNode)//找到后继关键字  
	{
		while (!pNode->isLeaf)
		{
			pNode = pNode->pChild[0];
		}
		return make_tuple(pNode->keyValue[0], pNode->position[0]);
	}
